...
M.on_attach = function(client, bufnr)
  ...
  if client.supports_method "textDocument/inlayHint" then
    ...
    if vim.lsp.inlay_hint then
      ...
      lsp_mappings.n["<leader>uH"] = {
        function() require("core.utils.ui").toggle_buffer_inlay_hints(bufnr) end,
  ...
  for id, _ in pairs(custom.lsp.progress) do -- clear lingering progress messages
    if not next(vim.lsp.get_active_clients { id = tonumber(id:match "^%d+") }) then custom.lsp.progress[id] = nil end
  end
  ...
  if server_name == "lua_ls" then -- by default initialize neodev and disable third party checking
    ...
    lsp_opts.before_init = function(param, config)
      if vim.b.neodev_enabled then
        for _, conf in ipairs(custom.supported_configs) do
          if param.rootPath:match(conf) then
